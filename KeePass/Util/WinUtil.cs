...
namespace KeePass.Util
{
    ...
    public static class WinUtil
    {
        ...
        public static byte[] HashFile(IOConnectionInfo iocFile)
        {
            ...
            Stream Load(IOConnectionInfo info)
            {
                try
                {
                    var sIn = IOConnection.OpenRead(info);
                    if (sIn is null) throw new FileNotFoundException();
                    return sIn;
                }
                catch (Exception) { return null; }
            }

            using Stream sIn = Load(iocFile);
        ...
        {
            using RegistryKey kNdp = Registry.LocalMachine.OpenSubKey(
            ...
            foreach (string strInNdp in vInNdp)
            {
                ...
                using RegistryKey kVer = kNdp.OpenSubKey(strInNdp, false);
                if (kVer != null)
                {
                    ...
                    foreach (string strProfile in vProfiles)
                    {
                        ...
                        using RegistryKey kPro = kVer.OpenSubKey(strProfile, false);
        ...
        private static void UpdateNetVersionFromRegKey(RegistryKey k, ref ulong uMaxVer)
        {
            ...
            try
            {
                ...
                string strInstall = k.GetValue("Install", "")!.ToString();
                if ((strInstall!.Length > 0) && (strInstall != "1")) return;

                string strVer = k.GetValue("Version", "")!.ToString();
                if (strVer!.Length > 0)
                {

...
namespace KeePassLib.Cryptography
{
    ...
    public sealed class CryptoRandom : IDisposable
    {
        ...
        public void AddEntropy(byte[] pbEntropy)
        {
...
#else
                using (var shaNew = new SHA512Managed())
#endif
                {
            ...
            lock (m_oSyncRoot)
            {
                var pbPool = m_pbEntropyPool.ReadData();
                var cbPool = pbPool.Length;
                var cbNew = pbNewData.Length;

                var pbCmp = new byte[cbPool + cbNew];
...
#else
                using (var shaPool = new SHA512Managed())
#endif
                {
                    var pbNewPool = shaPool.ComputeHash(pbCmp);
        ...
        private byte[] GenerateRandom256()
        {
            ...
            lock (m_oSyncRoot)
            {
                ...
                var pbCounter = MemUtil.UInt64ToBytes(m_uCounter);

                var pbCsp = GetCspRandom();

                var pbPool = m_pbEntropyPool.ReadData();
                var cbPool = pbPool.Length;
                var cbCtr = pbCounter.Length;
                var cbCsp = pbCsp.Length;
            ...
            var pbRet = CryptoUtil.HashSha256(pbCmp);
            MemUtil.ZeroByteArray(pbCmp);
            return pbRet;
        }
        ...
        public byte[] GetRandomBytes(uint uRequestedBytes)
        {
            ...
            if (uRequestedBytes > int.MaxValue)
            {
                Debug.Assert(false);
                throw new ArgumentOutOfRangeException("uRequestedBytes");
            }

            var cbRem = (int)uRequestedBytes;
            var pbRes = new byte[cbRem];
            var iPos = 0;
            ...
            while (cbRem != 0)
            {
                ...
                var cbCopy = Math.Min(cbRem, pbRandom256.Length);
        ...
        {
            var s64 = DateTime.UtcNow.ToBinary();
            var s32 = (int)((s64 >> 32) ^ s64);

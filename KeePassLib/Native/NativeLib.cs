...
namespace KeePassLib.Native
{
    ...
    public static class NativeLib
    {
            ...
            get
            {
                ...
                try
                {
                    ...
                    if (t != null)
                    {
                        ...
                        if (mi != null)
                        {
                            var strName = (mi.Invoke(null, null) as string);
        ...
        {
            var pDummy0 = new byte[32];
            var pDummy1 = new byte[32];

            // Save the native state
            var bCachedNativeState = m_bAllowNative;

            // Temporarily allow native functions and try to load the library
            m_bAllowNative = true;
            var bResult = TransformKey256(pDummy0, pDummy1, 16);
        ...
        public static DesktopType GetDesktopType()
        {
            if (!m_tDesktop.HasValue)
            {
                ...
                else
                {
                    try
                    {
                        var strXdg = (Environment.GetEnvironmentVariable(
                            "XDG_CURRENT_DESKTOP") ?? string.Empty).Trim();
                        var strGdm = (Environment.GetEnvironmentVariable(
...
#if (!KeePassLibSD && !KeePassUAP)
        public static string RunConsoleApp(string strAppPath, string strParams) => RunConsoleApp(strAppPath, strParams, null);

        public static string RunConsoleApp(string strAppPath, string strParams,
            string strStdInput) => RunConsoleApp(strAppPath, strParams, strStdInput,
            ...
            string strStdInput, AppRunFlags f)
        {
            ...
            var bStdOut = ((f & AppRunFlags.GetStdOutput) != AppRunFlags.None);
            ...
            {
                var lDisabledForms = new List<Form>();
                ...
                var strRet = fnRun.EndInvoke(ar);

                for (var i = lDisabledForms.Count - 1; i >= 0; --i)
        ...
        internal static void StartProcess(string strFile) => StartProcess(strFile, null);
        ...
        private static void CustomizeProcessStartInfo(ProcessStartInfo psi)
        {
            ...
            var vUrlEncSchemes = new string[] {
                "file:", "ftp:", "ftps:", "http:", "https:",
                "mailto:", "scp:", "sftp:"
            };
            foreach (var strPfx in vUrlEncSchemes)
            {
            ...
            if (IsUnix())
            {
                if (MonoWorkarounds.IsRequired(19836) && string.IsNullOrEmpty(strArgs))
                {
                    ...
                    {
                        var bMacOSX = (GetPlatformID() == PlatformID.MacOSX);

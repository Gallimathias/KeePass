...
namespace KeePassLib.Utility
{
    public static class MonoWorkarounds
    {
        ...
        public static bool IsRequired(uint uBugID)
        {
            ...
            if (g_dForceReq.TryGetValue(uBugID, out var bForce)) return bForce;

            var v = NativeLib.MonoVersion;
            if (v == 0) return true;
        ...
        internal static void SetEnabled(string strIDs, bool bEnabled)
        {
            ...
            var vIDs = strIDs.Split(new char[] { ',' });
            foreach (var strID in vIDs)
            {
                if (string.IsNullOrEmpty(strID)) continue;

                if (StrUtil.TryParseUInt(strID.Trim(), out var uID))
        ...
        private static void FixClipThread()
        {
            try
            {
                ...
                var strTest = ClipboardU.GetText();
        ...
        private static bool NeedClipboardWorkaround()
        {
            try
            {
                var strHandle = (NativeLib.RunConsoleApp(AppXDoTool,
                ...
                var strWmClass = (NativeLib.RunConsoleApp("xprop",
        ...
        private sealed class MwaHandlerInfo
        {
            ...
            public Delegate FunctionOriginal => m_fnOrg;

            private readonly Delegate m_fnOvr;
            public Delegate FunctionOverride => m_fnOvr;

            private readonly DialogResult m_dr;
            public DialogResult Result => m_dr;

            private readonly Form m_fContext;
            public Form FormContext => m_fContext;
        ...
        private static readonly Dictionary<object, MwaHandlerInfo> m_dictHandlers =
        ...
        private static void ApplyToButton(Button btn, Form fContext)
        {
            ...
            EventHandlerList ehl = GetEventHandlers(btn, out var objClickEvent);
            if (ehl == null) { Debug.Assert(false); return; }
            Delegate fnClick = ehl[objClickEvent]; // May be null

            var fnOvr = new EventHandler(MonoWorkarounds.OnButtonClick);
        ...
        private static void SetWmClass(Form f) => NativeMethods.SetWmClass(f, PwDefs.UnixName, PwDefs.ResClass);
        ...
        internal static bool ExchangeFormShownRaised(Form f, bool bNewValue)
        {
            ...
            try
            {
                ...
                var bPrevious = (bool)fi.GetValue(f);
        ...
        internal static void EnsureRecentlyUsedValid()
        {
            ...
            try
            {
                var strFile = Environment.GetFolderPath(
